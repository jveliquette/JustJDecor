...
def api_rooms(request):
    if request.method == "GET":
        ...
        return JsonResponse({"rooms": rooms}, encoder=RoomEncoder, safe=False)
    elif request.method == "POST":
        try:
            content = json.loads(request.body)
            room = Room.objects.create(**content)
            return JsonResponse(room, encoder=RoomEncoder, safe=False)
...
def api_projects(request):
    if request.method == "GET":
        ...
        try:
            ...
            return JsonResponse(project, encoder=ProjectEncoder, safe=False)
...
def api_wishlist_items(request):
    if request.method == "GET":
        ...
        return JsonResponse({"wishlist_items": wishlist_items}, encoder=WishlistItemEncoder, safe=False)
    elif request.method == "POST":
        try:
            content = json.loads(request.body)
            ...
            return JsonResponse(wishlist_item, encoder=WishlistItemEncoder, safe=False)
...
def api_pins(request):
    if request.method == "GET":
        ...
        return JsonResponse({"pins": pins}, encoder=PinEncoder, safe=False)
    elif request.method == "POST":
        try:
            content = json.loads(request.body)
            project_id = content.get("project")
            ...
            return JsonResponse(pin, encoder=PinEncoder, safe=False)
        except Project.DoesNotExist:
            return JsonResponse({"error": "Project does not exist"}, status=404)
        except Exception as e:
            return JsonResponse({"error": str(e)}, status=400)
...
def api_user_profiles(request):
    if request.method == "GET":
        ...
        return JsonResponse({"user_profiles": profiles}, encoder=UserProfileEncoder, safe=False)
    elif request.method == "POST":
        try:
            content = json.loads(request.body)
